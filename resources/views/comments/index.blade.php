                ...
                <div class="flex-none w-full max-w-full px-3">
                    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                        <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-between">
                            <h6 class="text-dark dark:text-white">Comments Table</h6>
                        </div>
                                ...
                                <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                                    <thead class="align-bottom">
                                        <tr>
                                            ...
                                            <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-size-xxs border-b-solid tracking-none whitespace-nowrap text-dark opacity-70">Comment</th>
                        ...
                        $.each(value.comments, function(key1, value1){
                            ...
                            dataHTML = dataHTML + `<tr>
                                    ...
                                    <td class="p-2 leading-normal text-center align-middle bg-transparent border-b dark:border-white/40 text-size-sm whitespace-nowrap shadow-transparent">
                                        <span class="{bgStatus} from-emerald-500 to-teal-400 px-3.6-em text-size-xs-em rounded-1.8 py-2.2-em inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none  text-dark">${value1.comment}</span>
                                    </td>
                                    <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                        <button onclick="deleteComment(${value1.id})" class="font-semibold leading-tight text-size-xs dark:text-white dark:opacity-80 bg-danger text-dark"> Hapus </button>
                                    </td>
                                </tr>`
                        })
                    })
                    if (noComment){
                        dataHTML = dataHTML + `<div class="text-center">no comment found</div>`
                    }
                    $('#loadingPosts').empty()
                    $('#comments').empty()
